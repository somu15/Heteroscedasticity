...
gamma_cov_pri = diag(diag(gamma_cov_pri))+10^10*[10 0 0;0 10 0;0 0 10^10];
...
% ylabel('Gamma 3')
% subplot(3,2,6)
sa_range = 0.05:0.05:1.5;
exact = importdata('C:\Users\lakshd5\Dropbox\Bayesian_IM_selection\Accounting for heteroskedasticity\data\Roof Drift\Exact_stds_1.5g.txt');
reg_para = importdata('C:\Users\lakshd5\Dropbox\Bayesian_IM_selection\Accounting for heteroskedasticity\data\Roof Drift\reg.txt');
plot(sa_range,sqrt((exp(parameters(3)+parameters(4)*(sa_range)+parameters(5)*(sa_range).^2))),'red','linewidth',1.5)
hold on
plot(sa_range,sqrt(exp(parameters_bayes(3)+parameters_bayes(4)*(sa_range)+parameters_bayes(5)*(sa_range).^2)),sa_range,sqrt((exp(reg_para(1)+reg_para(2)*(sa_range)+reg_para(3)*(sa_range).^2))),'g','linewidth',1.5)%
hold on
...
clc
file_path  = 'C:\Users\lakshd5\Dropbox\Bayesian_IM_selection\Accounting for heteroskedasticity\data\Roof Drift\';
Sa = importdata(strcat(file_path,'Sa_80.txt'));
%Sa = Sa(1:40);
Dr = importdata(strcat(file_path,'Dr_80.txt'));
%Dr = Dr(find(Sa<1.1));
%Sa = Sa(find(Sa<1.1));
X = [ones(max(size(Sa)),1) log(Sa)];
...
cov_matrix = inv(A_0);
beta_cov_pri = cov_matrix(1:2,1:2);
gamma_cov_pri = cov_matrix(3:max(size(parameters)),3:max(size(parameters)))+[1 0;0 10^10];
...
% hold on
% scatter(Sa,sqrt(K),'*')
sa_range = 0.05:0.05:1;
plot(sa_range,sqrt((exp(parameters(3)+parameters(4)*(sa_range)))),'red','linewidth',1.5)
hold on
plot(sa_range,sqrt(exp(parameters_bayes(3)+parameters_bayes(4)*(sa_range))),sa_range,sqrt((exp(-2.576+1.003*(sa_range)-0.5159*(sa_range).^2))),'green','linewidth',1.5)%
hold on
exact = importdata('C:\Users\lakshd5\Dropbox\Bayesian_IM_selection\Accounting for heteroskedasticity\data\Roof Drift\Exact_stds.txt');
%ex = interp1(exact(:,1),exact(:,2),sa_range);
% scatter(exact(:,1),exact(:,2))
% hold on
scatter(exact(:,1),exact(:,2))
...
%rFit = rsquare(exact(:,2),sqrt((exp(-3.83+2.91*(exact(:,1))-1.02*(exact(:,1)).^2))));
SSE_Fit = sum((exact(:,2)'-sqrt((exp(-2.576+1.003*(sa_range)-0.5159*(sa_range).^2)))).^2);
% rBayes
% rFreq
% rFit
SSE_Bayes
